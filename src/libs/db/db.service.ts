...
export type RepoData = { db: string; tbn: string; tmodel: Repo; repo: Repo; rid: string };
...
export class DBService {
	...
	public async getRepoByOptions({ db, tbn, tmodel }: RepoOptions) {
		...
		if (!repo.$options) repo.$options = { rid: `${db}.${tbn}`, db, tbn, tmodel, repo };
		return repo as Repo;
	}
	public getRepoData(r: Repo | Model | RepoOptions | any): any {
		...
		//
		let db = typeof r.db === 'string' ? r.db : r.db.getDatabaseName();
		let tbn = r.tbn;
		let sequelize = this.getDBConnection(db);
		if (sequelize.isDefined(tbn)) {
			let options: any = { db, tbn };
